...
def showTreat():
    ...
    try:
        if os.path.getsize('./ttt/doc_ttt23/convdose.json'):
            subprocess.call('./ttt/doc_ttt23/tabs.py')
...
patient 23, convdose.json file missing !") 
...
def showReserve():
    ...
    try:
        if os.path.getsize('./ttt/doc_ttt23/convres.json'):
            subprocess.call('./ttt/doc_ttt23/tabres.py')
    except FileNotFoundError as no_tabs:
...
patient 23, convres.json file missing !")
...
def copyToFile():
    ...
    to reuse them after.
    """
    with open('./ttt/doc_ttt23/intro_ttt.txt', '+a') as file:
        file.write(str("Date : "))
...
def copyToReserve():
    ...
    to reuse them after.
    """
    with open('./ttt/doc_ttt23/intro_res.txt', '+a') as file:
        file.write(str("Date : "))
        file.write(textDate.get() + '\n')
        file.write(str("Heure : "))
        file.write(textHour.get() + '\n')
...
with open('./allergy/allergyfile23.txt', 'r') as filename2:
