...
def showTreat():
    ...
    try:
        if os.path.getsize('./ttt/doc_ttt12/convdose.json'):
            subprocess.call('./ttt/doc_ttt12/tabs.py')
...
patient 12, convdose.json file missing !") 
...
def showReserve():
    ...
    try:
        if os.path.getsize('./ttt/doc_ttt12/convres.json'):
            subprocess.call('./ttt/doc_ttt12/tabres.py')
    except FileNotFoundError as no_tabs:
...
patient 12, convres.json file missing !")
...
def copyToFile():
    ...
    to reuse them after.
    """
    with open('./ttt/doc_ttt12/intro_ttt.txt', '+a') as file:
        file.write(str("Date : "))
...
def copyToReserve():
    ...
    to reuse them after.
    """
    with open('./ttt/doc_ttt12/intro_res.txt', '+a') as file:
        file.write(str("Date : "))
        file.write(textDate.get() + '\n')
        file.write(str("Heure : "))
        file.write(textHour.get() + '\n')
...
with open('./allergy/allergyfile12.txt', 'r') as filename2:
