...
def showTreat():
    ...
    try:
        if os.path.getsize('./ttt/doc_ttt20/convdose.json'):
            subprocess.call('./ttt/doc_ttt20/tabs.py')
...
patient 20, convdose.json file missing !") 
...
def showReserve():
    ...
    try:
        if os.path.getsize('./ttt/doc_ttt20/convres.json'):
            subprocess.call('./ttt/doc_ttt20/tabres.py')
    except FileNotFoundError as no_tabs:
...
patient 20, convres.json file missing !")
...
def copyToFile():
    ...
    to reuse them after.
    """
    with open('./ttt/doc_ttt20/intro_ttt.txt', '+a') as file:
        file.write(str("Date : "))
...
def copyToReserve():
    ...
    to reuse them after.
    """
    with open('./ttt/doc_ttt20/intro_res.txt', '+a') as file:
        file.write(str("Date : "))
        file.write(textDate.get() + '\n')
        file.write(str("Heure : "))
        file.write(textHour.get() + '\n')
...
with open('./allergy/allergyfile20.txt', 'r') as filename2:
