...
def showTreat():
    ...
    try:
        if os.path.getsize('./ttt/doc_ttt15/convdose.json'):
            subprocess.call('./ttt/doc_ttt15/tabs.py')
...
patient 15, convdose.json file missing !") 
...
def showReserve():
    ...
    try:
        if os.path.getsize('./ttt/doc_ttt15/convres.json'):
            subprocess.call('./ttt/doc_ttt15/tabres.py')
    except FileNotFoundError as no_tabs:
...
patient 15, convres.json file missing !")
...
def copyToFile():
    ...
    to reuse them after.
    """
    with open('./ttt/doc_ttt15/intro_ttt.txt', '+a') as file:
        file.write(str("Date : "))
...
def copyToReserve():
    ...
    to reuse them after.
    """
    with open('./ttt/doc_ttt15/intro_res.txt', '+a') as file:
        file.write(str("Date : "))
        file.write(textDate.get() + '\n')
        file.write(str("Heure : "))
        file.write(textHour.get() + '\n')
...
with open('./allergy/allergyfile15.txt', 'r') as filename2:
