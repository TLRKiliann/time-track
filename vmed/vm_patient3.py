...
class Application(Frame):
    ...
    def lienDirect(self):
        try:
            if os.path.getsize('./vmed/doc_vmed3/resultvmed.txt'):
                print("+ File 'VMED3' exist (add)!")
                subprocess.call('./vmed/doc_vmed3/vmed_write.py')
        except FileNotFoundError as outmsg:
            print("+ Sorry, file 'VMED3' not exist !", outmsg)
            print("+ File VMED3 created !")
            with open('./vmed/doc_vmed3/resultvmed.txt', 'w') as file:
                file.write(".")
            self.confRec()
    ...
    def lectureFic(self):
        try:
            if os.path.getsize('./vmed/doc_vmed3/resultvmed.txt'):
                print("+ File 'VMED3' exist (read)!")
                subprocess.call('./vmed/doc_vmed3/vmed_read.py')
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'VMED3' not exist !", outcom)
            with open('./vmed/doc_vmed3/resultvmed.txt', 'w') as file:
                file.write(".")
            self.confRec()
