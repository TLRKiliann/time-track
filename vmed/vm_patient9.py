...
class Application(Frame):
    ...
    def lienDirect(self):
        try:
            if os.path.getsize('./vmed/doc_vmed9/resultvmed.txt'):
                print("+ File 'VMED9' exist (add)!")
                subprocess.call('./vmed/doc_vmed9/vmed_write.py')
        except FileNotFoundError as outmsg:
            print("+ Sorry, file 'VMED9' not exist !", outmsg)
            print("+ File VMED9 created !")
            with open('./vmed/doc_vmed9/resultvmed.txt', 'w') as file:
                file.write(".")
            self.confRec()
    ...
    def lectureFic(self):
        try:
            if os.path.getsize('./vmed/doc_vmed9/resultvmed.txt'):
                print("+ File 'VMED9' exist (read)!")
                subprocess.call('./vmed/doc_vmed9/vmed_read.py')
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'VMED9' not exist !", outcom)
            with open('./vmed/doc_vmed9/resultvmed.txt', 'w') as file:
                file.write(".")
            self.confRec()
