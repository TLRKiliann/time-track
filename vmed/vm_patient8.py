...
class Application(Frame):
    ...
    def lienDirect(self):
        try:
            if os.path.getsize('./vmed/doc_vmed8/resultvmed.txt'):
                print("+ File 'VMED8' exist (add)!")
                subprocess.call('./vmed/doc_vmed8/vmed_write.py')
        except FileNotFoundError as outmsg:
            print("+ Sorry, file 'VMED8' not exist !", outmsg)
            print("+ File VMED8 created !")
            with open('./vmed/doc_vmed8/resultvmed.txt', 'w') as file:
                file.write(".")
            self.confRec()
    ...
    def lectureFic(self):
        try:
            if os.path.getsize('./vmed/doc_vmed8/resultvmed.txt'):
                print("+ File 'VMED8' exist (read)!")
                subprocess.call('./vmed/doc_vmed8/vmed_read.py')
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'VMED8' not exist !", outcom)
            with open('./vmed/doc_vmed8/resultvmed.txt', 'w') as file:
                file.write(".")
            self.confRec()
