...
class Application(Frame):
    ...
    def lienDirect(self):
        try:
            if os.path.getsize('./vmed/doc_vmed12/resultvmed.txt'):
                print("+ File 'VMED12' exist (add)!")
                subprocess.call('./vmed/doc_vmed12/vmed_write.py')
        except FileNotFoundError as outmsg:
            print("+ Sorry, file 'VMED12' not exist !", outmsg)
            print("+ File VMED12 created !")
            with open('./vmed/doc_vmed12/resultvmed.txt', 'w') as file:
                file.write(".")
            self.confRec()
    ...
    def lectureFic(self):
        try:
            if os.path.getsize('./vmed/doc_vmed12/resultvmed.txt'):
                print("+ File 'VMED12' exist (read)!")
                subprocess.call('./vmed/doc_vmed12/vmed_read.py')
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'VMED12' not exist !", outcom)
            with open('./vmed/doc_vmed12/resultvmed.txt', 'w') as file:
                file.write(".")
            self.confRec()
