...
class Application(Frame):
    ...
    def lienDirect(self):
        try:
            if os.path.getsize('./vmed/doc_vmed17/resultvmed.txt'):
                print("+ File 'VMED17' exist (add)!")
                subprocess.call('./vmed/doc_vmed17/vmed_write.py')
        except FileNotFoundError as outmsg:
            print("+ Sorry, file 'VMED17' not exist !", outmsg)
            print("+ File VMED17 created !")
            with open('./vmed/doc_vmed17/resultvmed.txt', 'w') as file:
                file.write(".")
            self.confRec()
    ...
    def lectureFic(self):
        try:
            if os.path.getsize('./vmed/doc_vmed17/resultvmed.txt'):
                print("+ File 'VMED17' exist (read)!")
                subprocess.call('./vmed/doc_vmed17/vmed_read.py')
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'VMED17' not exist !", outcom)
            with open('./vmed/doc_vmed17/resultvmed.txt', 'w') as file:
                file.write(".")
            self.confRec()
