...
class Application(Frame):
    ...
    def lienDirect(self):
        try:
            if os.path.getsize('./vmed/doc_vmed10/resultvmed.txt'):
                print("+ File 'VMED10' exist (add)!")
                subprocess.call('./vmed/doc_vmed10/vmed_write.py')
        except FileNotFoundError as outmsg:
            print("+ Sorry, file 'VMED10' not exist !", outmsg)
            print("+ File VMED10 created !")
            with open('./vmed/doc_vmed10/resultvmed.txt', 'w') as file:
                file.write(".")
            self.confRec()
    ...
    def lectureFic(self):
        try:
            if os.path.getsize('./vmed/doc_vmed10/resultvmed.txt'):
                print("+ File 'VMED10' exist (read)!")
                subprocess.call('./vmed/doc_vmed10/vmed_read.py')
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'VMED10' not exist !", outcom)
            with open('./vmed/doc_vmed10/resultvmed.txt', 'w') as file:
                file.write(".")
            self.confRec()
