    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile19.txt'):
                print("+ File 'allergyfile19.txt' exist !")
                with open('./allergy/allergyfile19.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile19.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile19.txt' created !")
            with open('./allergy/allergyfile19.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
