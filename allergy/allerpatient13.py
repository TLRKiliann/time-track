    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile13.txt'):
                print("+ File 'allergyfile13.txt' exist !")
                with open('./allergy/allergyfile13.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile13.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile13.txt' created !")
            with open('./allergy/allergyfile13.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
