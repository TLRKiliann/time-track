    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile24.txt'):
                print("+ File 'allergyfile24.txt' exist !")
                with open('./allergy/allergyfile24.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile24.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile24.txt' created !")
            with open('./allergy/allergyfile24.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
