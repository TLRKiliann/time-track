    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile20.txt'):
                print("+ File 'allergyfile20.txt' exist !")
                with open('./allergy/allergyfile20.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile20.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile20.txt' created !")
            with open('./allergy/allergyfile20.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
