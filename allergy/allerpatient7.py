    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile7.txt'):
                print("+ File 'allergyfile7.txt' exist !")
                with open('./allergy/allergyfile7.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile7.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile7.txt' created !")
            with open('./allergy/allergyfile7.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
