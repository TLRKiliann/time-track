    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile3.txt'):
                print("+ File 'allergyfile3.txt' exist !")
                with open('./allergy/allergyfile3.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile3.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile3.txt' created !")
            with open('./allergy/allergyfile3.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
