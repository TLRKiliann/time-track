    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile21.txt'):
                print("+ File 'allergyfile21.txt' exist !")
                with open('./allergy/allergyfile21.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile21.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile21.txt' created !")
            with open('./allergy/allergyfile21.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
