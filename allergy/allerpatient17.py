    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile17.txt'):
                print("+ File 'allergyfile17.txt' exist !")
                with open('./allergy/allergyfile17.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile17.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile17.txt' created !")
            with open('./allergy/allergyfile17.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
