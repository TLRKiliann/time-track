    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile14.txt'):
                print("+ File 'allergyfile14.txt' exist !")
                with open('./allergy/allergyfile14.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile14.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile14.txt' created !")
            with open('./allergy/allergyfile14.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
