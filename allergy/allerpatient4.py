    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile4.txt'):
                print("+ File 'allergyfile4.txt' exist !")
                with open('./allergy/allergyfile4.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile4.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile4.txt' created !")
            with open('./allergy/allergyfile4.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
