    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile12.txt'):
                print("+ File 'allergyfile12.txt' exist !")
                with open('./allergy/allergyfile12.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile12.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile12.txt' created !")
            with open('./allergy/allergyfile12.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
