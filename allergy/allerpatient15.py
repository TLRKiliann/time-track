    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile15.txt'):
                print("+ File 'allergyfile15.txt' exist !")
                with open('./allergy/allergyfile15.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile15.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile15.txt' created !")
            with open('./allergy/allergyfile15.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
