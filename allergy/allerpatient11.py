    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile11.txt'):
                print("+ File 'allergyfile11.txt' exist !")
                with open('./allergy/allergyfile11.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile11.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile11.txt' created !")
            with open('./allergy/allergyfile11.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
