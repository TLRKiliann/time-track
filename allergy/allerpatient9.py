    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile9.txt'):
                print("+ File 'allergyfile9.txt' exist !")
                with open('./allergy/allergyfile9.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile9.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile9.txt' created !")
            with open('./allergy/allergyfile9.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
