    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile10.txt'):
                print("+ File 'allergyfile10.txt' exist !")
                with open('./allergy/allergyfile10.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile10.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile10.txt' created !")
            with open('./allergy/allergyfile10.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
