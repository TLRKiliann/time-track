    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile2.txt'):
                print("+ File 'allergyfile2.txt' exist !")
                with open('./allergy/allergyfile2.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile2.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile2.txt' created !")
            with open('./allergy/allergyfile2.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
