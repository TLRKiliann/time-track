    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile18.txt'):
                print("+ File 'allergyfile18.txt' exist !")
                with open('./allergy/allergyfile18.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile18.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile18.txt' created !")
            with open('./allergy/allergyfile18.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
