    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile16.txt'):
                print("+ File 'allergyfile16.txt' exist !")
                with open('./allergy/allergyfile16.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile16.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile16.txt' created !")
            with open('./allergy/allergyfile16.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
