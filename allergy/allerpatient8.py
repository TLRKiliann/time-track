    ...
    if MsgBox == 1:
        ...
        try:
            if os.path.getsize('./allergy/allergyfile8.txt'):
                print("+ File 'allergyfile8.txt' exist !")
                with open('./allergy/allergyfile8.txt', 'a+') as namefile:
                    namefile.write(entryall.get() + '\n')
                    namefile.write(str('----------------\n'))
                    gui.destroy()
        except FileNotFoundError as outcom:
            print("+ Sorry, file 'allergyfile8.txt' not exist !")
            print(str(outcom))
            print("+ File 'allergyfile8.txt' created !")
            with open('./allergy/allergyfile8.txt', 'a+') as namefile:
                namefile.write(entryall.get() + '\n')
                namefile.write(str('----------------\n'))
                gui.destroy()
